...
class TFSimulator(simulator.SimulatorFlexibleGeometries):
  ...
  def update_temperature_estimates(
    ...
    def _get_denominator(
      ...
      # --- CP² FIX (denominator) - currently DISABLED (original code active) ---
      ...
      dt3 = tf.math.multiply(dt3, t_heat_capacity)  # ORIGINAL: cp applied twice
      # dt3 = pass  # FIX: comment out line above to remove cp²
      dt3 = tf.math.divide(dt3, t_delta_t)
      # --- END CP² FIX (denominator) ---
    ...
    def _get_numerator(
      ...
      # Create the thermal absorption term.
      # --- CP² FIX (numerator) - currently DISABLED (original code active) ---
      ...
      nt3 = tf.math.multiply(nt3, t_heat_capacity)  # ORIGINAL: cp applied twice
      # nt3 = pass  # FIX: comment out line above to remove cp²
      nt3 = tf.math.multiply(nt3, t_temp_minus)
      nt3 = tf.math.divide(nt3, t_delta_t)
      # --- END CP² FIX (numerator) ---
