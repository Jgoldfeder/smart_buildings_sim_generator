...
class StochasticConvectionSimulator(
  ...
  def __init__(self, p: float, distance: int, seed: Optional[int], every_k_steps: int = 1):
    ...
    Args:
      ...
      seed: random seed
      every_k_steps: only shuffle once every k calls to apply_convection
    """
    self._p = p
    self._distance = distance
    self._every_k_steps = max(1, every_k_steps)
  ...
  def apply_convection(
    ...
    if self._step_counter % self._every_k_steps != 0:
      return
